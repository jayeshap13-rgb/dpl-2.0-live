...
function initFirebaseSync() {
  if (!firebaseReady()) {
    console.warn("Firebase scripts are not available. Website is using local browser data only.");
    return;
  }
  try {
    ...
    firebaseConnected = false;
    console.error(error);
    if (route === "admin") toast("Firebase setup failed. Website is using local browser data only.");
  }
}
function queueFirebaseSave() {
  if (!firebaseDoc || applyingFirebaseData) return;
...
async function pushFirebaseData() {
  ...
  try {
    ...
    firebaseConnected = false;
    console.error(error);
    if (route === "admin") toast("Could not publish online. Firestore may be blocked or the data may be too large.");
  }
}
...
function handleClick(event) {
  ...
  const adminActions = new Set([
    ...
    "reset-data", "export-json", "publish-online",
  ]);
  ...
  if (action === "publish-online") {
    pushFirebaseData().then(() => {
      toast(firebaseConnected ? "Current data published online" : "Could not publish online");
      render();
    });
  }
